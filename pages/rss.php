...
if ('' == $func) {
    ...
    $query = 'SELECT `id`, `title`, `url`, `table` FROM ' . rex_yfeed::table() . ' WHERE `type` = "rss" ORDER BY `table`, `title`, `url`';
    $list = rex_list::factory($query);
    $list->addTableAttribute('class', 'table-striped');

    $tdIcon = '<i class="rex-icon rex-icon-rss"></i>';
    ...
    $fragment->setVar('title', $this->i18n('rss'));
    ...
    $field = $form->addHiddenField('type', 'rss');
    ...
    $field->getValidator()->add('notEmpty', $this->i18n('yfeed_url_empty'));
